...
export const  AcceptRequests=()=>{
    ...
    useEffect(() => {  
      ...
      useEffect(()=>{
          const fetchRequests=async()=>{
              try{
                  const response=await axios.get("https://ewalletfullstack.onrender.com/api/v1/account/getAllrequests",{
                      withCredentials:true,
                      headers:{
                          "Content-Type":"application/json",
                      },
    ...
    const handleGive =(request) => {
        try{
            const response=axios.post("https://ewalletfullstack.onrender.com/api/v1/account/approve",{
                to:request.user._id,
